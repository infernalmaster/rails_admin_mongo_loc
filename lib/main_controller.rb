  ...
  class MainController < RailsAdmin::ApplicationController
    ...
    def sanitize_params_for!(action, model_config = @model_config, _params = params[@abstract_model.param_key])
      ...
      allowed_methods = fields.map{|f|
        f.allowed_methods
      }.flatten.uniq.map(&:to_s)
      #create array of FILELD_translations
      #this method will do it for EVERY field, not only localized
      #todo fix it, should create array of only localized fields
      transl_methods = allowed_methods.map{|f|
        f + '_translations'
      }
      allowed_methods += transl_methods
      allowed_methods << "id" << "_destroy"
